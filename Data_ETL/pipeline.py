...
from Data_ETL.extract import extract
from Data_ETL.transform import transform_data
from Data_ETL.upload_database import upload_with_error_handling, db_config
from Data_ETL.read_database import read_data
...
def run_pipeline(input_file, output_path):
    ...
    logger.info(f"Starting ETL pipeline: {input_file} -> {output_path}")
    ...
    df_addedid = add_unique_id_simple(output_df_clean, id_column='Breach ID', prefix='DS', start=1)

    filename = os.path.basename(input_file)
    df_addedid['File Name'] = filename

    final_columns = ['File Name', 'Breach ID', 'Name', 'Emp ID', 'DOB', 'SSN']
...
if __name__ == "__main__":
    ...
    result_df = run_pipeline(input_file, output_path)
