...
def generate_remarks(ref_info, chk_info, sheet_configs):
    ...
    Returns list of remark strings.
    """
    remarks = []
    all_pass = True
    ...
    for cfg in sheet_configs:
        ...
        if ref["name"] != chk["name"]:
            remarks.append(
                f'{sheet_label}: Sheet name is incorrect — expected "{ref["name"]}", found "{chk["name"]}".'
            )
            all_pass = False

        ref_headers = ref["headers"]
        chk_headers = chk["headers"]

        # Trim trailing empty columns from both sides for length comparison
        ref_trimmed = [h for h in ref_headers if h.strip() != ""]
        chk_trimmed = [h for h in chk_headers if h.strip() != ""]

        # Check for extra blank columns appended
        if len(chk_headers) > len(ref_headers):
            extra = len(chk_headers) - len(ref_headers)
            blank_extras = [h for h in chk_headers[len(ref_headers):] if h.strip() == ""]
            named_extras = [h for h in chk_headers[len(ref_headers):] if h.strip() != ""]
            if blank_extras:
                remarks.append(
                    f'{sheet_label}: {len(blank_extras)} additional blank column(s) detected beyond the expected header range.'
                )
                all_pass = False
            for h in named_extras:
                remarks.append(
                    f'{sheet_label}: Unexpected column "{h}" found beyond the expected header range.')
                all_pass = False

        # Check each expected column
        for idx, ref_h in enumerate(ref_trimmed):
            if idx >= len(chk_headers):
                remarks.append(
                    f'{sheet_label}: Column "{ref_h}" (position {idx + 1}) is missing.')
                all_pass = False
            elif chk_headers[idx].strip() == "":
                remarks.append(
                    f'{sheet_label}: Column position {idx + 1} is blank; expected "{ref_h}".')
                all_pass = False
            elif chk_headers[idx].strip() != ref_h.strip():
                remarks.append(
                    f'{sheet_label}: Column "{ref_h}" (position {idx + 1}) is incorrect — found "{chk_headers[idx].strip()}".'
                )
                all_pass = False
